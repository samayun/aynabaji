...
const shouldBareClone = packageJson.mirror['bare:pull'];
const shouldMirrorPush = packageJson.mirror['mirror:push'];
const shouldTakeGift = packageJson.mirror['takeGift'];

const setRemoteOrigin = packageJson.mirror['setRemoteOrigin'];
...
Promise.all(
  REPOSITORIES.map(([PROJECT, MIRROR_PROJECT]) => {
    ...
    if (setRemoteOrigin) {
      console.log(Object.entries(setRemoteOrigin));

      Object.entries(setRemoteOrigin).map(([LOCAL_DIRECTORY, MIRROR_PROJECT]) => {
        const REMOTE_ORIGIN_SET_CMD = `git remote set-url origin  git@${MIRROR_PLATFORM}.com:${MIRROR_ORGANIZATION_SLUG}/${MIRROR_PROJECT}.git`;
